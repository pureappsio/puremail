...
// Segment
Router.route("/api/segment", { where: "server" }).get(function() {

    var query = this.params.query;
    console.log(query);

    // Check there is at least subscriber ID and sequence
    if (query.subscriber && query.sequence) {

        // Get data
        var sequence = Sequences.findOne(query.sequence);

        // Assign sequence
        Meteor.call('assignSequenceManual', query.subscriber, query.sequence);

        // Add tags if any
        if (query.tag) {

            var subscriber = Subscribers.findOne(query.subscriber);

            // Current tags
            var tags = subscriber.tags;
            if (tags.indexOf(query.tag) == -1) {
                tags.push(query.tag);
            }

            // Update
            Subscribers.update(subscriber._id, { $set: { tags: tags } });
        }

        // Find thank you page
        if (sequence.thankYou != "") {
            var target = sequence.thankYou;
        } else {
            var list = Lists.findOne(sequence.listId);
            var target = list.thankYou;
        }
...
Router.route("/api/subscribers/:email", { where: "server" }).get(function() {
    ...
    if (Meteor.call('validateApiKey', user, key)) {
        ...
        // Try to get by IP
        else if (Subscribers.findOne({ip: subscriberEmail})) {

            var subscriber = Subscribers.findOne({ip: subscriberEmail});
